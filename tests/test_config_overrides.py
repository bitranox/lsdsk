...
from lsdsk.adapters.config.overrides import (
    ConfigOverride,
    apply_overrides,
    coerce_value,
    parse_override,
...
# --------------------------------------------------------------------------
# The default view is a view, so configuration has to reach it too
# --------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ("override", "marker"),
    [
        ("thresholds.wear_warning_percent", "endurance"),
        ("display.summary_limit", "more, run"),
    ],
)
def test_configuration_reaches_the_bare_default_view(
    cli_runner: CliRunner, production_factory: Callable[[], Any], override: str, marker: str
) -> None:
    """It resolved no tunables at all, so every configured value was ignored.

    Every subcommand calls ``resolve_tunables``; the root group called
    ``resolve_history`` alone and let ``run_default_report`` fall back to the
    shipped defaults for both sections. Measured: with
    ``thresholds.wear_warning_percent`` set to 1 and then 99, ``lsdsk findings``
    produced 22 endurance findings and then none, while the bare view produced
    none either way. This is the view the tool tells you to run first.
    """
    from lsdsk.adapters.cli import cli

    fixture = Path(__file__).parent / "fixtures" / "hw" / "linux-sas-hba.json"
    low, high = ("1", "99") if "thresholds" in override else ("2", "5")

    outputs: list[str] = []
    for value in (low, high):
        result = cli_runner.invoke(
            cli, ["--set", f"{override}={value}", "--replay", str(fixture)], obj=production_factory
        )
        assert result.output, f"{override}={value}: the bare view produced nothing"
