...
def test_when_profile_is_valid_alphanumeric_it_accepts(clear_config_cache: None) -> None:
    ...
    # `is not None` cannot fail here, because get_config returns Config. What is
    # actually being asserted is that a name with a hyphen and a digit passes
    # validation, so assert against the validator's own verdict on a name that
    # does NOT, which is the pair that can tell them apart.
